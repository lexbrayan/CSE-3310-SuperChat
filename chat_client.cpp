...
public:
  ...
  void updateRoomWindow(string roomname[], int participant_no[], int crn)
  {
    int i;
    scrollok(room_window, TRUE);
    start_color();
    init_pair(1, COLOR_RED, COLOR_BLACK);
    wprintw(room_window, "\n");
    for (i = 0; i < 10; i++)
    {
      if (TRUE) //you can replace this sentence so that only chatrooms that are created get printed.
      {
        if (i == crn)
        {
          attron(COLOR_PAIR(1)); //coloring does not work
          attron(A_BOLD);
          wprintw(room_window, "|%s\t(%d)\n", roomname[i].c_str(), participant_no[i]);
...
*/
  ...
  int main(int argc, char *argv[])
  {
    try
    {
      ...
      startx = 0;
      starty = 0;
      room_window = newwin(height1 + height2, room_width, starty, startx + width1);
      box(room_window, 0, 0);
      wrefresh(room_window);

      //TESTING PURPOSES DELETE THE POPULATING LATER
      string roomname[10];
      int participant_no[10];
      roomname[0] = "Lobby";
      roomname[1] = "One";
      roomname[2] = "Two";
      roomname[3] = "Three";
      ...
      participant_no[8] = 80;
      participant_no[9] = 90;
      //TESTING POPULATION DONE

      c.updateRoomWindow(roomname, participant_no,chat_room_number); //Prints to the room window all the roomnames and their participant numbers
